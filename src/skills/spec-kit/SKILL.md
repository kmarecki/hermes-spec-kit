...
- **Additive-safe editing**. Never delete or reorder existing entries in bugs.md, tasks.md, plan.md, spec.md, clarify.md, close.md, or implementation-summary.md. You MAY modify existing entries — update status fields, correct text — but only the minimal field or section needed. Never restructure or refactor a document beyond the change. New entries always append at the end. Preserve every existing bug ID, task ID, and requirement.
