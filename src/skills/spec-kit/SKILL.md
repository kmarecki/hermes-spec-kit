...
- **Full workflow for new bugs**: When "bugfix [feature]" is invoked after new bugs were logged mid-round, the workflow MUST run the COMPLETE plan → tasks → implement cycle for those bugs. Plan and tasks each get their OWN commit (never batched together, never batched with code). Plan: `spec(phase-2): [feature] bugfix plan (BUG-NNN, ...)`. Tasks: `spec(phase-3): [feature] bugfix tasks (BUG-NNN, ...)`. Implement: per fix. This applies even for single-line fixes — the phase sequence is mandatory.
...
- **Bugfix sub-rounds: plan + tasks are NEVER batched.** When new bugs are discovered mid-round and fixed in a subsequent round, plan sections get their own commit, tasks get their own commit. Batching them loses phase-level traceability and makes `git log --grep="spec(phase-2)"` miss bugfix plan entries.
...
- **Bugfix sub-round** (new bugs discovered mid-round and fixed in a subsequent round): Plan and tasks each get their OWN commit (never batched). Plan: `spec(phase-2): [feature] bugfix plan (BUG-NNN, ...)`. Tasks: `spec(phase-3): [feature] bugfix tasks (BUG-NNN, ...)`. Implementation commits per fix, separately.
